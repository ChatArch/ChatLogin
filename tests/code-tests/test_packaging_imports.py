...
def test_distribution_exports_new_public_apis():
    script = "import chatlogin as cl; assert cl.__version__ == '0.1.3'; assert cl.AsyncCallbackBackend; assert cl.AsyncCredentialBackend"
...
def test_core_import_does_not_load_optional_web_modules():
    script = (
        ...
        "assert chatlogin.__version__ == '0.1.3'"
    )
    subprocess.run([sys.executable, "-c", script], cwd="/tmp", check=True)
