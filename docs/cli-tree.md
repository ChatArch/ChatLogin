...
这篇文档是 `ChatLogin` CLI 的简明能力地图，用来校对哪些命令已经是一等入口、哪些仍然只是边界或规划。生成后请按真实命令树更新；不要把未实现命令写成已可用操作。
...
```text
chatlogin --help           # 验证命令已安装，并查看当前命令树
chatlogin --version        # 验证当前安装版本
chatlogin --tree           # 回读带参数签名的真实 CLI contract
chatlogin --tree-brief     # 回读不含参数签名的简明命令树
```
...
```text
chatlogin <group>          # 按当前包真实能力命名的命令组
├── <command>                  # 说明这个命令做什么
