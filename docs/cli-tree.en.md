...
This page is the compact capability map for the `ChatLogin` CLI. Use it to review which commands are first-class entries and which are still boundary or planned slots. After scaffolding, update it with the real command tree; do not present unimplemented commands as available operations.
...
```text
chatlogin                  # ChatLogin command-line entry
...
```text
chatlogin --help           # Verify the command is installed and inspect the current command tree
chatlogin --version        # Verify the installed version
chatlogin --tree           # Read back the CLI contract with parameter signatures
chatlogin --tree-brief     # Read back command nodes and descriptions only
```
...
```text
chatlogin <group>          # Command group named after real package capability
