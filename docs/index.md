...
ChatLogin 是面向网站集成的 ChatArch Python 包。当前版本提供标准包结构、真实 CLI 树与 ChatEnv 扩展入口；登录、用户管理和鉴权功能尚未实现。此处记录当前可用接口，不把规划写成已实现能力。
...
英文首页见站点语言入口：<https://arch.gh.wzhecnu.cn/ChatLogin/en/>。缺少英文翻译的专题页会按 i18n fallback 回退到中文页面。
