...
## Unreleased — 0.1.6.dev2 多用户与界面修订预览

- 四种后端均提供A/B两个独立合成用户，登录表单可选择并填入。
- 新增宿主内存样例的本人读写及双向越权拒绝，真实调用`require_owner`，不接受客户端owner字段；用户身份与数据授权责任分开说明。
- 账号切换与退出继续复用现有CSRF契约；私有异步结果随认证generation失效，不重新填回已清空的页面。
- 同步中英文多用户能力、演示操作及逻辑隔离边界；不更改真实消费方账号或业务数据。

- 收紧首页与表单标题，优先展示可交互登录示例。
- 修复中间宽度卡片裁切；手机模板选项与效果同屏，代码按需展开。
- 外观切换直接更新现有预览，不刷新iframe或清空输入。
- 演示资源增加构建版本URL，避免旧CSS/JS缓存掩盖修复。
